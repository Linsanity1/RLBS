...
def _isParallelogram(four_points: _FourPointSet):
    """
    Whether the 4 points form an parallelogram.
    """
    two_points_combination = combinations(four_points, 2)
    shortest_dist_squared = _compute_shortest_dist_squared(four_points)

    mid_point_list = []
    for two_points in list(two_points_combination):
        mid_point_list.append(_compute_mid_point(two_points[0], two_points[1]))

    if _parallelogram_test(mid_point_list, shortest_dist_squared / 10):
        return True
    else:
        return False
